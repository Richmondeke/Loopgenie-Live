...
import { getVoices } from '../services/heygenService';
import { HeyGenVoice, YouTubeChannelPreferences } from '../types';
...
export const ChannelMaker: React.FC<ChannelMakerProps> = ({ userCredits, onBack, onGenerate, onSaveChannel, initialChannel }) => {
    ...
    const [preferences, setPreferences] = useState<YouTubeChannelPreferences>({
        captionStyle: 'BOXED',
        voiceId: 'Fenrir',
        aspectRatio: '9:16',
        animationStyle: 'ZOOM'
        ...
        const fetchVoices = async () => {
            ...
            if (apiKey) {
                try {
                    ...
                    console.warn("Failed to fetch HeyGen voices:", e);
                }
            }
        };
        fetchVoices();
    ...
    const runProduction = async () => {
        ...
        for (const ep of episodesToProduce) {
            try {
                ...
                // TRIGGER DOWNLOAD IMMEDIATELY (Local Blob URL is fastest)
                updateEpProgress(ep.id, { step: '📥 Downloading...', percent: 96, lastLog: 'Triggering browser download...' });
                triggerDownload(videoUrl, `loopgenie_${channel.name}_${ep.title.replace(/\s+/g, '_')}.mp4`);

                updateEpProgress(ep.id, { step: '☁️ Uploading...', percent: 97, lastLog: 'Uploading to storage (background)...' });
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-10 shadow-sm border border-gray-100 dark:border-gray-700 animate-in fade-in slide-in-from-bottom-8 duration-700">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    <div>
                                        <h3 className="text-lg font-black text-gray-900 dark:text-white mb-4">Series Settings</h3>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            {/* Frame Size */}
                                            <div>
                                                <label className="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1">Frame Size</label>
                                                <div className="flex gap-2">
                                                    {[
                                                        { id: '9:16', label: 'Vertical', icon: <Layout size={14} className="rotate-90" /> },
                                                        { id: '16:9', label: 'Wide', icon: <Layout size={14} /> },
                                                        { id: '1:1', label: 'Square', icon: <div className="w-3.5 h-3.5 border-2 border-current rounded-sm" /> }
                                                    ].map((ratio) => (
                                                        <button
                                                            key={ratio.id}
                                                            onClick={() => setPreferences(prev => ({ ...prev, aspectRatio: ratio.id as any }))}
                                                            className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${preferences.aspectRatio === ratio.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-gray-50 dark:bg-gray-900 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                                                        >
                                                            {ratio.icon}
                                                            {ratio.label}
                                                        </button>
                                            ...
                                            {/* Animation Style */}
                                            <div>
                                                <label className="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1">Animation Style</label>
                                                <div className="flex gap-2">
                                                    {(['ZOOM', 'PAN', 'STATIC'] as const).map((style) => (
                                                        <button
                                                            key={style}
                                                            onClick={() => setPreferences(prev => ({ ...prev, animationStyle: style }))}
                                                            className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${preferences.animationStyle === style ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-gray-50 dark:bg-gray-900 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                                                        >
                                                            {style}
                                                        </button>
                                                    ))}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {/* Voiceover */}
                                        <div>
                                            <label className="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1">Voiceover</label>
                                            <select
                                                value={preferences.voiceId}
                                                onChange={(e) => setPreferences(prev => ({ ...prev, voiceId: e.target.value }))}
                                                className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold text-gray-900 dark:text-white outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none"
                                            >
                                                <option value="Fenrir">Default (Fenrir)</option>
                                        ...
                                        {/* Captions */}
                                        <div>
                                            <label className="block text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1">Captions</label>
                                            <select
                                                value={preferences.captionStyle}
                                                onChange={(e) => setPreferences(prev => ({ ...prev, captionStyle: e.target.value as any }))}
                                                className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold text-gray-900 dark:text-white outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none"
                                            >
                                                <option value="BOXED">Boxed (Recommended)</option>
