    ...
    return (
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50">
            ...
            <div className="flex gap-2">
                <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="p-1 rounded hover:bg-gray-200 disabled:opacity-50"><ChevronLeft size={16} /></button>
                <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-1 rounded hover:bg-gray-200 disabled:opacity-50"><ChevronRight size={16} /></button>
            </div>
        </div>
    );
};
    ...
    if (selectedUser) {
        ...
        return (
            <div className="max-w-7xl mx-auto pb-10">
                <button onClick={handleBackToUsers} className="flex items-center gap-2 text-gray-500 hover:text-gray-900 mb-6 font-medium transition-colors"><ChevronLeft size={20} /> Back to Users</button>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm md:col-span-1">
                        <div className="flex flex-col items-center text-center">
                            <div className="w-20 h-20 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 mb-4"><User size={40} /></div>
                            ...
                            <div className="w-full space-y-3 mt-2">
                                <div className="flex justify-between items-center text-sm p-3 bg-gray-50 rounded-lg"><span className="flex items-center gap-2 text-gray-600"><CreditCard size={14}/> Balance</span><span className="font-bold text-indigo-600">{selectedUser.credits_balance} Credits</span></div>
                                <div className="flex justify-between items-center text-sm p-3 bg-gray-50 rounded-lg"><span className="flex items-center gap-2 text-gray-600"><Activity size={14}/> Total Spent</span><span className="font-bold text-gray-900">{userCreditsUsed} Credits</span></div>
                                <div className="flex justify-between items-center text-sm p-3 bg-gray-50 rounded-lg"><span className="flex items-center gap-2 text-gray-600"><Calendar size={14}/> Joined</span><span className="font-medium text-gray-900">Unknown</span></div>
                            </div>
                        </div>
                    </div>
                    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm md:col-span-2 flex flex-col overflow-hidden">
                        <div className="p-4 border-b border-gray-100 bg-gray-50 font-bold text-gray-700 flex justify-between items-center"><span>User Activity History</span><span className="text-xs font-normal text-gray-500">{userProjects.length} Projects</span></div>
                                            ...
                                            <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50">
                                                <td className="px-6 py-3">
                                                    <div className="flex items-center gap-3 cursor-pointer group" onClick={() => handleOpenMedia(p)}>
                                                        ...
                                                        <div className="font-medium text-gray-900 truncate max-w-[150px] group-hover:text-indigo-600 transition-colors">{p.templateName}</div>
                                                    </div>
                                                </td>
    ...
    return (
        <div className="max-w-7xl mx-auto pb-10">
            <div className="flex items-center justify-between mb-8">
                <div><h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2"><ShieldCheck className="text-indigo-600" /> Admin Dashboard</h1><p className="text-gray-500">System Overview & User Management</p></div>
                <div className="flex bg-gray-100 p-1 rounded-lg">
                    {['OVERVIEW', 'USERS', 'ACTIVITY'].map((tab) => (
                        <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === tab ? 'bg-white shadow text-indigo-700' : 'text-gray-500 hover:text-gray-700'}`}>{tab.charAt(0) + tab.slice(1).toLowerCase()}</button>
                    ))}
                </div>
            </div>
                ...
                <div className="space-y-8 animate-in fade-in duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-4 mb-2"><div className="p-3 bg-blue-50 text-blue-600 rounded-xl"><Users size={24} /></div><div><div className="text-xs font-bold text-gray-400 uppercase">Total Users</div><div className="text-2xl font-black text-gray-900">{totalUsers}</div></div></div>
                            <div className="text-xs text-green-600 font-bold bg-green-50 inline-block px-2 py-1 rounded">+{activeUsers} Active recently</div>
                        </div>
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-4 mb-2"><div className="p-3 bg-green-50 text-green-600 rounded-xl"><DollarSign size={24} /></div><div><div className="text-xs font-bold text-gray-400 uppercase">Est. Revenue</div><div className="text-2xl font-black text-gray-900">${estimatedRevenue.toFixed(2)}</div></div></div>
                            <div className="text-xs text-gray-500 font-medium">From {totalCreditsUsed} Credits</div>
                        </div>
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-4 mb-2"><div className="p-3 bg-orange-50 text-orange-600 rounded-xl"><Activity size={24} /></div><div><div className="text-xs font-bold text-gray-400 uppercase">Est. Provider Cost</div><div className="text-2xl font-black text-gray-900">${estimatedProviderCost.toFixed(2)}</div></div></div>
                            <div className="text-xs text-gray-500 font-medium">Est. Margin: <span className="font-bold text-green-600">${estimatedProfit.toFixed(2)}</span></div>
                        </div>
                         <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-4 mb-2"><div className="p-3 bg-red-50 text-red-600 rounded-xl"><Activity size={24} /></div><div><div className="text-xs font-bold text-gray-400 uppercase">Failed Jobs</div><div className="text-2xl font-black text-gray-900">{totalFailed}</div></div></div>
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {paginatedUsers.map(user => (
                                    <tr key={user.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors group cursor-pointer" onClick={() => handleUserClick(user)}>
                                        ...
                                        <tr key={project.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3 cursor-pointer group" onClick={() => handleOpenMedia(project)}>
                                                    ...
                                                    <div className="font-medium text-gray-900 truncate max-w-[150px] group-hover:text-indigo-600 transition-colors">{project.templateName}</div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-gray-600">{project.user_email || 'Unknown'}</td>
                                            <td className="px-6 py-4"><span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-600">{project.type}</span></td>
                                            <td className="px-6 py-4 font-bold text-gray-700">{project.cost || 1} Cr (${revenue.toFixed(2)})</td>
                                            <td className="px-6 py-4 text-gray-500">${providerCost.toFixed(2)}</td>
                                            <td className="px-6 py-4 font-bold text-green-600">+${margin.toFixed(2)}</td>
                                            <td className="px-6 py-4 text-right text-gray-500 text-xs">{new Date(project.createdAt).toLocaleDateString()}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
